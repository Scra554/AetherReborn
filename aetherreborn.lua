--[[
╔══════════════════════════════════════════════════════════════════╗
║              aetherreborn RESUMMON — Universal Keyless Hub               ║
...
-- ═══════════════════════════════════════════════════════════════
if _G.aetherrebornRunning then
    _G.aetherrebornRunning = false
    wait(0.15)
end
if _G.aetherrebornConnections then
    for _, c in ipairs(_G.aetherrebornConnections) do pcall(function() c:Disconnect() end) end
end
if _G.aetherrebornDrawings then
    for _, d in ipairs(_G.aetherrebornDrawings) do pcall(function() d:Remove() end) end
end
if _G.aetherrebornHighlights then
    for _, h in ipairs(_G.aetherrebornHighlights) do pcall(function() h:Destroy() end) end
end
for _, c in ipairs(game:GetService("CoreGui"):GetChildren()) do
    if c.Name == "ui" or c.Name:find("aetherreborn") then pcall(function() c:Destroy() end) end
end

_G.aetherrebornRunning     = true
_G.aetherrebornConnections = {}
_G.aetherrebornDrawings    = {}
_G.aetherrebornHighlights  = {}
...
local function nD(t, p) local d = Drawing.new(t); for k,v in pairs(p or {}) do d[k]=v end; table.insert(_G.aetherrebornDrawings, d); return d end
...
local function mkESP(p)
    ...
    sb.Parent = workspace
    d.SB = sb
    table.insert(_G.aetherrebornHighlights, sb)
    ESPObjects[p] = d
end
...
local function enableNoclip()
    ...
    table.insert(_G.aetherrebornConnections, noclipConn)
end
...
local function setupAntiFling()
    ...
    bv.Parent   = hrp
    _G.aetherrebornAntiFlingBV = bv
end
local function disableAntiFling()
    if _G.aetherrebornAntiFlingBV then
        pcall(function() _G.aetherrebornAntiFlingBV:Destroy() end)
        _G.aetherrebornAntiFlingBV = nil
    end
end
...
local mainConn = RunService.RenderStepped:Connect(function(dt)
    if not _G.aetherrebornRunning then return end
...
table.insert(_G.aetherrebornConnections, mainConn)
...
table.insert(_G.aetherrebornConnections, jumpConn)
...
table.insert(_G.aetherrebornConnections, moveConn)
...
table.insert(_G.aetherrebornConnections, afkConn)

-- Player cleanup
table.insert(_G.aetherrebornConnections, Players.PlayerRemoving:Connect(rmESP))
...
local Window  = VapeLib:Window("aetherreborn Resummon", Color3.fromRGB(0, 200, 255), Enum.KeyCode.RightShift)
...
MoveTab:Button("Save Teleport Position", function()
    ...
    if root then
        ...
        print("[aetherreborn] TP position saved:", root.Position)
    end
end)
...
MoveTab:Button("Teleport Back to Saved Position", function()
    if Cfg.TpBackPos then
        ...
        print("[aetherreborn] No saved position!")
    end
end)
...
UtilTab:Button("Copy JobId to Clipboard", function()
    ...
    if setclipboard then
        setclipboard(id)
        print("[aetherreborn] Copied JobId:", id)
    else
        print("[aetherreborn] JobId:", id)
    end
end)
...
UtilTab:Button("Server Hop (Smallest Server)", function()
    ...
    local ok, result = pcall(function()
        ...
        if #servers > 0 then
            ...
            print("[aetherreborn] No other servers found.")
        end
    end)
    if not ok then print("[aetherreborn] Server hop error:", result) end
end)
...
UtilTab:Button("Save Config to Clipboard", function()
    ...
    print("[aetherreborn] Config saved to clipboard!")
end)
...
SetTab:Button("Reset All Movement", function()
    ...
    print("[aetherreborn] Movement reset.")
end)

SetTab:Button("Unload aetherreborn Resummon", function()
    _G.aetherrebornRunning = false
    for _, c in ipairs(_G.aetherrebornConnections) do pcall(function() c:Disconnect() end) end
    for _, d in ipairs(_G.aetherrebornDrawings) do pcall(function() d:Remove() end) end
    for _, h in ipairs(_G.aetherrebornHighlights) do pcall(function() h:Destroy() end) end
    ...
    for _, c in ipairs(CoreGui:GetChildren()) do
        if c.Name == "ui" or c.Name == "aetherrebornSnow" then pcall(function() c:Destroy() end) end
    end
    print("[aetherreborn Resummon] Unloaded cleanly.")
end)
